...
Before I move on though, I do want to add one note of caution, one that I forgot to include in the original version of this post: the NHANES study isn't a simple random sample, and the sampling design is complicated. As such, care is required in how the data are used and this often requires taking the sample weights into account. I haven't done so here, because this post is already exhaustingly long and the topic is nuanced enough that it deserves to be its own post, but I'll mention that [accounting for study design](https://wwwn.cdc.gov/nchs/nhanes/tutorials/sampledesign.aspx) is discussed on the NHANES website. (Also, thanks to both Thomas Lumley and Benjamin Rich for asking about this issue, which reminded me that I hadn't actually talked about this and prompted me to add this paragraph!)
