...
babygotback <- read_lines("babygotback.txt")
```

## Warning

Let's start this descent into madness with a disclaimer: I have *never* wanted to write a primer on working with regular expressions in R. I loathe regular expressions, and I am not good at them. About 99% of my knowledge of regular expressions comes from weeping in abject despair with a dozen R documentation tabs open and desperately clicking on random links at [www.regular-expressions.info](https://www.regular-expressions.info) to work out why my regex doesn't work. I am, very clearly, not an expert. 
...
This post is getting long. We need a little break. Well, I need a little break. Anyway, here's the entirety of the "Midnight Pals" dialog with all words hissed by JK Rowling detected by regex:

```{r}
#| column: page
midnight |> 
  str_view(
    pattern = "\\b((\\w|')*s{3,}(\\w|')*)|(yess)\\b", 
    match = NA
  ) |> 
  print(n = Inf)
```

For a variety of different reasons this makes me smile. But let's move on, shall we?
...
But now suppose that we want to relax this slightly and find all words that start and end with the same (case-matched) letter, regardless of what that letter actually is. Here's the point at which we need to use a backreference:
...
Oh... by the way? This trick won't work in base R unless you set `perl = TRUE`.^[That's another one of those fun things where there are in fact three separate flavours of regex in widespread usage in R. ICU (used by stringi and stringr) and PCRE (base R with `perl = TRUE`) both support lookahead and lookbehind assertions, as well as negative lookahead `(?!blah)` and negative lookbehind `(?<!blah)`, but the ERE engine (base R with `perl = FALSE`) does not.] Which seems like a good time to discuss...
...
```{r}
base_extract_all(punct, "[[:punct:]]") |> unlist() |> cat()
```

What about PCRE? Let's set `perl = TRUE` and have a look:

```{r}
base_extract_all(punct, "[[:punct:]]", perl = TRUE) |> unlist() |> cat()
```
...
```{r}
str_extract_all(punct, "[[:punct:]]") |> unlist() |> cat()
```
...
However, there are some fancy things that you can do with replacement text. In particular, all three regular expression engines used in R support backreferences within the replacement text. Here it is in tidyverse:

```{r}
#| results: hold
# tidyverse + ICU
cat(str_replace_all(
  string = "dogs are better than cats",
  pattern = "(dogs)(.*)(cats)",
  replacement = "\\3\\2\\1"
), sep = "\n")
