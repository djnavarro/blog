...
Now that we've talked about a lot more regular expresasion concepts, and in doing so introduced a lot more special characters, it's worth revisiting that list of reminders about how to use backslashes. To save you (i.e., me in six months time) the effort of flicking back and forth between the two sections, I'll start by reiterating the previous list. Additionally, because -- as I've hinted -- the rules *inside* a character set are different to the rules that apply *outside* a character set, I'll handle those two cases separately.
...
In all these examples I'm using `str_view()`, so we're using the "tidyverse style" regular expressions here. In base R, if you set `perl = TRUE` you'd get the same results here. If you don't, however, it's a bit different. If you're using the default base R regex flavour, the only character that you can escape within a character set is the backslash itself, so if you want to match `]`, `^`, or `-` then you have to rely on the positional tricks shown above. 
