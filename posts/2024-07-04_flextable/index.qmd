...
streaks <- function(seed, n = 10) {
  ...
  ggplot(
    ...
    geom_segment(linewidth = 2, show.legend = FALSE) + 
...
```{r}
flextable(df) %>% 
  set_header_labels(seed = "RNG Seed", plot = "Streaks Plot") %>% 
  compose(
    j = "plot", 
    value = as_paragraph(
      gg_chunk(value = plot, width = 4, height = .3)
    )
  ) %>% 
  autofit()
```
