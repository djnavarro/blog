...
In practice, about 90% of my usage of the fs package is via the `path()` function used to construct file paths, so it's the natural place to start. Here's a very simple example that specifies the path from my blog root to this quarto document:
