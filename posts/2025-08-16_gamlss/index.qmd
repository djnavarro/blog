...
We can take the logic further and predict the distribution of weights for tall (95th height percentile) and short (5th height percentile) people, stratified by age and sex:
...
Sample from a trimmed Box-Cox power-exponential: because even with nu/tau parameters, the BCPE has support on non-biological values in the tails, creating physically impossible outliers in large simulations. the default trim is 1% on either side, (as compared to 3% for `nhanesgamlss::simwtage()`)
...
Sample height conditional on `age`, using an appropriate (sec-specific) gamlss model `ht_mod`. Similarly, we can sample weight conditional on `age` and `height` using a model.

```{r}
sample_h <- function(age, ht_mod) {
  pars <- get_pars(
    data = tibble(age_mn = age),
    model = ht_mod
  )
  pars |> 
...
sample_w <- function(age, height, wt_mod) {
  pars <- get_pars(
    data = tibble(age_mn = age, height_cm = height),
    model = wt_mod
  )
  pars |> 
    select(mu, sigma, nu, tau) |> 
...
```{r}
dat <- expand_grid(age_mn = 1:24, sex_fct = c("male", "female"))
sample_hw(age = dat$age_mn, sex = dat$sex_fct, mod = mod)
```