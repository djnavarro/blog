...
A small slice through a very large data set, but one that contains the four variables that I need in almost every NHANES-based analysis: `age_mn` ($a$), `sex_fct` ($s$), `height_cm` ($h$), and `weight_kg` ($w$). I can now turn to the substantive statistical problem: estimating the joint conditional density $p(h, w | a, s)$.
...
$$
\tilde{y}_i = s_0 + \sum_{k=1}^p f_k(x_{ik})
$$
...
$$
g(\tilde{y}_i) = s_0 + \sum_{k=1}^p f_k(x_{ik})
$$
...
If we replace the regression coefficients $\beta_{\mu k}$ and $\beta_{\sigma k}$ with smooth functions $f_{\mu k}()$ and $f_{\sigma k}()$ and use these to transform the predictors, we now have an additive regression model for the mean and the standard deviation...
