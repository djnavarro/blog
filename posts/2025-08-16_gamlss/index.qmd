...
Crudely put, the GLM applies a linear model to a transformed **outcome** variable (i.e., $y$). On the other hand, in the [additive model](https://en.wikipedia.org/wiki/Additive_model) proposed by [Friedman and Stuetzle (1981)](https://www.tandfonline.com/doi/abs/10.1080/01621459.1981.10477729), transformation functions (often called "smoothing functions") are applied to the **predictor** variables (i.e., $x$):

$$
\tilde{y}_i = s_0 + \sum_{k=1}^p s_k(x_{ik})
$$

When you do both of these things, the end result is the [generalised additive model](https://en.wikipedia.org/wiki/Generalized_additive_model) (GAM) was introduced by [Hastie and Tibshirani (1990)](https://www.jstor.org/stable/2245459), in which

$$
g(\tilde{y}_i) = s_0 + \sum_{k=1}^p s_k(x_{ik})
$$
...
When expressed this way, the "homogeneity of variance" assumption in linear model corresponds to a model in which no covariates (predictors) are included in the model for $\sigma$. However there's no inherent reason to limit ourselves in this way. Relaxing homogeneity of variance allows us to specify regression models for both $\mu$ and $\sigma$,
...
Introducing smoothing functions $s()$ to transform the predictors gives us this:

$$
\begin{array}{rcl}
\mu_i &=& s_{\mu 0} + \sum_k s_{\mu k}(x_{ik}) \\
\sigma_i &=& s_{\sigma 0} + \sum_k s_{\sigma k}(x_{ik}) \\ \\
y_i &\sim& \mbox{Normal}(\mu_i, \sigma_i)
\end{array}
$$

The normal distribution is not the only choice of measurement model. Like many other distributions (e.g., beta distribution) it can be described as a distribution that contains one parameter for the **location** (i.e., $\mu$) and **scale** (i.e., $\sigma$). More generally though, we might want to use distributions described by one parameter for location, one parameter for scale, and one or more parameters for the **shape**. If we instead choose the [Box-Cox power exponential](/posts/2025-08-02_box-cox-power-exponential/), the statistical model now looks like this,
...
\sigma_i &=& s_{\sigma 0} + \sum_k s_{\sigma k}(x_{ik}) \\
\nu_i &=& s_{\nu 0} + \sum_k s_{\nu k}(x_{ik}) \\
\tau_i &=& s_{\tau 0} + \sum_k s_{\tau k}(x_{ik}) \\ \\
y_i &\sim& \mbox{BCPE}(\mu_i, \sigma_i, \nu_i, \tau_i)
\end{array}
$$

and we now have a **generalised additive model for location, scale, and shape** (GAMLSS).
