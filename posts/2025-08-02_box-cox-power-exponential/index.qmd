...
It's pretty clear that you can interpret $\mu$ and $\sigma$ with little difficulty, but interpreting the two shape parameters $\nu$ and $\tau$ requires some care. I'll return to this later. For now, let's just move forward and repeat the fitting exercise for the female body weight data: 
...
Just to be sure that this has worked, let's plot the estimated BCPE densities (red curves) on top of a simple histogram-based density estimate (grey bars) for these two data sets:
