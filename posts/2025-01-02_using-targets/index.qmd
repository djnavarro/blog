...
The blog has now been built (you can view it [here](./site-1/index.html)), and the project folder now looks like this:
...
Two new folders have been added: `_targets` stores metadata and tracked objects for the pipeline, and `site` contains the built website. 
...
Okay, time to talk about the [dynamic branching](https://books.ropensci.org/targets/dynamic.html). You'll probably have noticed that although the `_targets.R` file *defines* only the one target called `post_files`, in the section of code that reads like this...
...
Okay yeah but I didn't actually have to scan over the output, I could have just called `tar_branches()`:

```{r}
tar_branches(post_files)
```

What are these branches? Let's find out:
...
```{r}
post_file_bfe2f <- tar_read(post_files_bfe2f528519c0ff0)
post_file_bfe2f
```
