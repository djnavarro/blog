...
Let's modify the source for the 404 page. I'll change it so that the title of the page now reads "404 PAGE NOT FOUND" rather than simply "404" as it had previously. 

Okay... yep, done. I've edited the file now. 

How does this edit get picked up in the targets build process? This time around, I'll build it in stages so that the output is a little easier to read. First, let's build the `post_files` target again:

```{r}
tar_make(post_files)
```

You can see that one (and only one) of the `post_files` branches has been rebuilt, specifically the one corresponding to the `404.rmd` file that we just edited. Okay, next let's see what happens when `post_fuse` is rebuilt:

```{r}
tar_make(post_fuse)
```

Because `post_paths` is set to always run, it does indeed run, but nothing has changed. As a consequence, none of the downstream `post_files` branches need updating (not surprising, since I literally just updated them in the previous line of code!) Nevertheless, one of the posts (i.e., the modified `404.rmd` file) has not yet been fused into an HTML file. So that one branch is dispatched by targets, and the `404.html` file in the site folder has now been updated appropriately. 
...
Again, let's see how this plays out in the build process:
