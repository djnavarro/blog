...
2. Declare that *every* R markdown document should be a tracked file. This is the role of the `post_files` target, but this is actually many targets bundled together (see below). 
3. For each tracked R markdown document, use `blog$post_fuse()` to build the post and move the resulting HTML file to the correct location. This is the role of `post_fuse`, and again this bundles many targets together.

The lower pipeline is analogous but simpler:
...
At this point, you probably (hopefully!) have a general sense of what the build process looks like for this blog. I haven't explained all the details yet, but I hope the gist is clear already. So -- kicking the can down the road just a little further -- let's ignore those tiresome details and call `tar_make()` and build the blog. The output is long and tedious, but there's not much in there that we didn't already see earlier with the D&D spells project: 
...
Setting aside obvious questions like "what's going on with all those weirdly named targets like `post_files_bfe2f528519c0ff0`???", the gist of this is pretty clear: everything has run smoothly and the blog has been built. In fact, you can see a live version of what this version of the blog looks like (i.e., when the source files are as described up to this point in thhis post) [here](./site-1/index.html)). After the blog has been built, the project folder now looks like this:
...
Two new folders have been added: `_targets` stores metadata and tracked objects for the pipeline, and `site` contains the built website. Yay! It works!
...
One of the problems I ran into pretty early in this project was how to monitor the source directory properly. It's easy enough to list the contents of a directory, of course: the `$find_posts()` and `$find_static()` methods for `Liteblog` objects do exactly this. But consider a target like this:
...
This target stores a list of all R markdown posts, but it has only one dependency: the `blog` object itself. It is *not* dependent on the current state of the source folder. It can't be: a folder in the file system isn't an R object and cannot itself be a target. So, if the state of the folder changes (e.g., a new blog post is added), targets would have no way of knowing that the `post_paths` target is now outdated. Moreover, since the contents of the folder can change at any time (even when R isn't running), it doesn't make a lot of sense to ask targets to constantly monitor the folder.

A simpler solution is to declare that the `post_paths` target should never be considered up to date, and must always be run whenever `tar_make()` is called. We can do this by using [`tar_cue()`](https://docs.ropensci.org/targets/reference/tar_cue.html), and that's why the actual target is defined like this:
...
A similar approach is taken when defining the `static_paths` target. These two targets are always executed at build time, which makes a lot of sense for a project like this one. What this means, in practice, is that even though I have just run `tar_make()` and haven't changed any of the files since then, the network visualisation appears like this: 
...
Okay, I've put it off as long as I could, but the time has come where I have to talk about [dynamic branching](https://books.ropensci.org/targets/dynamic.html). You'll probably have noticed that although the `_targets.R` file *defines* only the one target called `post_files`, in the section of code that reads like this...
...
this corresponds to several *tracked* objects. Sifting through the output of calling `tar_make()` we see that `post_files` actually corresponds to the following four "branches"...

```
post_files_bfe2f528519c0ff0
post_files_0d76c076472d293c
post_files_64b628c4152d280c
post_files_31d6f026c8bea126
```

Of course, I didn't actually have to do this manually. Instead, I could have called [`tar_branches()`](https://docs.ropensci.org/targets/reference/tar_branches.html):

```{r}
pf_branches <- tar_branches(post_files)
pf_branches
```

As you can see by comparison to the manual listing above, the first column lists the four branches of the `post_files` target. 

```{r}
tar_read_raw(pf_branches$post_files[1])
```
...
Let's modify the source for the 404 page:
...
What does that do in terms of target outdating?
...
You can browse the built website [here](./site-2/index.html)
