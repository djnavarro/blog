...
worker_lookup <- dplyr::inner_join(
  x = crew, 
  y = trace_sum, 
  by = dplyr::join_by(targets, dplyr::closest(x$seconds > y$seconds))
) |> 
  dplyr::select(worker, worker_pid)

dplyr::left_join(trace, worker_lookup, by = "worker_pid")
```
