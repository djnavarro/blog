...
![Somehow, "Palpatine" returned.](poe.png)
...
People will be amazed at my precognitive powers and my apparently-encyclopedic knowledge of all things computational. They will praise my genius, my devotees will raise me up on a palanquin, and I shall live as a queen and goddess amongst the tiresome mortals...
...
Then we write a `sampler()` function:
...
Okay it works, but I really wouldn't use this implementation in the wild because my version is pretty lazy: I'm only doing this so that I can convince myself that I understand what's going on, and also to separate the sampler from the transformer for simulation purposes. Speaking of which...

## Simulations

Okay now comes the fun part. How shall we generate some covariance matrices to play around with? Well, let's start with three observations: 
...
We can now look at the sensitivity of the transformation matrices to perturbations added to the (probably evil) covariance matrix that we pass to our sampler. 
...
for (i in 1:nrow(out)) {
  ...
  out$error[i] <- all(diffs < .01 | a1_vec == 0)
}

library(ggplot2)
ggplot(out, aes(factor(error), kappa)) +
  geom_violin(draw_quantiles = .5) +
  theme_bw() + 
  facet_wrap(~method) + 
  scale_y_log10() +
  labs(
    x = "has the transformation matrix changed?",
    y = "condition number for matrix inversion"
  )
```

I really love this plot. I mean, on the one hand it's kind of stupid if you're trying to extract nuance from it, because there is no nuance to be found here. The problem exists in MASS but not mvtnorm, and it has bugger all to do with "standard" condition numbers. There are literally only two things to see here. But on the other hand, it's pretty brutal about ramming home those two points, right?
